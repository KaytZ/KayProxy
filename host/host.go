...
func appendToHost(hostPath string) error {
	content := " \n# KayProxy（Go）\n"
	for domain := range common.HostDomain {
...
func backupHost(hostPath string) (bool, error) {
	...
	for {
		...
		if !containsProxyDomain {
			if strings.Contains(strings.ToUpper(newLine), strings.ToUpper("KayProxy")) {
				containsProxyDomain = true
				log.Println("Found KayProxy Line")
			}
			for domain := range common.ProxyDomain {
...
func excludeRelatedHost(hostPath string) error {
	...
	for {
		...
		if needWrite && strings.Contains(strings.ToUpper(newLine), strings.ToUpper("KayProxy")) {
			needWrite = false
		}
...
func resolveIp() (ip string, err error) {
